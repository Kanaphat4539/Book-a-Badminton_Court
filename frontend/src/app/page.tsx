    ...
    return (
      <div className="bg-gradient-to-br from-orange-50 via-white to-orange-100 text-on-surface antialiased min-h-screen flex flex-col pt-24 pb-24 font-sans relative overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto space-y-6 w-full px-container-padding mt-4 relative z-10">
          <div className="flex justify-between items-center bg-white/80 backdrop-blur-xl p-6 rounded-3xl shadow-lg border border-white/60">
            <div>
              <h1 className="font-display-sm text-[28px] font-extrabold text-gray-900 drop-shadow-sm">Admin Dashboard</h1>
              <p className="font-body-md text-[15px] text-gray-600 font-medium">Manage courts and view all bookings</p>
            </div>
          ...
          <section>
            <h2 className="font-headline-md text-[22px] font-extrabold mb-4 text-gray-900">All Bookings</h2>
            <div className="space-y-4">
              {allBookings.length === 0 && <p className="text-gray-500 font-medium text-[15px] bg-white/50 p-6 rounded-2xl text-center">No bookings found.</p>}
              {allBookings.map(booking => (
                <div key={booking.id} className="bg-white/70 backdrop-blur-xl p-5 rounded-2xl flex justify-between items-center border border-white/60 shadow-[0_4px_16px_rgba(0,0,0,0.03)] hover:shadow-[0_8px_24px_rgba(0,0,0,0.06)] transition-all">
                  <div>
                    <p className="font-headline-md text-[18px] font-bold text-gray-900">{booking.user?.name} <span className="text-gray-500 font-semibold text-[14px]">(@{booking.user?.username})</span></p>
                    <p className="text-[15px] text-gray-600 mt-1 font-medium">{booking.court?.name} • {booking.booking_date}</p>
                  </div>
                  <div className="text-right flex flex-col items-end gap-2">
                    ...
                    <span className={`text-[11px] font-bold px-3 py-1 rounded-full uppercase tracking-wider ${
                      booking.status === 'COMPLETED' ? 'bg-emerald-100 text-emerald-700 border border-emerald-200' :
  ...
  return (
    <div className="bg-gradient-to-br from-orange-50 via-white to-orange-100 text-on-surface antialiased min-h-screen flex flex-col pt-24 pb-24 font-sans relative overflow-hidden">
      ...
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none z-0">
        ...
        <div className="absolute top-[40%] right-[-10%] w-[400px] h-[400px] bg-yellow-200/50 rounded-full mix-blend-multiply filter blur-3xl opacity-70"></div>
      ...
      <main className="max-w-5xl mx-auto space-y-8 w-full px-container-padding mt-6 relative z-10">
        ...
        {/* Welcome Header */}
        <section>
          <h1 className="font-display-lg text-[36px] font-extrabold mb-1 text-gray-900 drop-shadow-sm tracking-tight">Hi, {user.name}</h1>
          <p className="font-body-lg text-[17px] text-gray-600 font-medium">Welcome back to KMITL Badminton</p>
        </section>

        {/* Active Session Card */}
        {activeBooking && (
          <section className="bg-white/80 backdrop-blur-xl rounded-3xl p-6 md:p-8 border-2 border-primary/30 shadow-[0_12px_32px_rgba(255,107,0,0.15)] relative overflow-hidden transform hover:-translate-y-1 transition-transform duration-300">
            ...
            <div className="relative z-10">
              ...
              <p className="font-body-md text-[15px] text-gray-700 font-medium mb-6">You are checked in to <span className="font-bold text-gray-900">{activeBooking.court?.name}</span></p>
              
              <div className="text-center py-6 bg-white/60 backdrop-blur-md rounded-2xl border border-white shadow-inner">
                <p className="font-label-md text-[13px] font-bold text-gray-500 mb-2 tracking-widest">TIME REMAINING (ENDS AT {activeBooking.end_time.slice(0,5)})</p>
        ...
        {pendingBooking && !activeBooking && (
          <section className="bg-white/70 backdrop-blur-xl rounded-3xl p-6 md:p-8 border border-white/60 shadow-[0_8px_32px_rgba(0,0,0,0.06)] relative overflow-hidden transform hover:-translate-y-1 transition-transform duration-300">
             <div className="absolute top-0 right-0 w-48 h-48 bg-gradient-to-bl from-blue-400/20 to-transparent rounded-full blur-2xl -mr-10 -mt-10 pointer-events-none"></div>
             <div className="relative z-10">
                <h2 className="font-headline-md text-[20px] font-extrabold mb-1 text-gray-900">Upcoming Booking</h2>
                <p className="font-body-md text-[15px] text-gray-600 font-medium mb-6">Your next court reservation</p>

                <div className="flex justify-between items-center mb-6 bg-white/60 p-5 rounded-2xl border border-white shadow-sm">
                  <div>
                    <p className="font-headline-md text-[18px] font-extrabold text-primary">{pendingBooking.court?.name}</p>
                    <p className="font-body-md text-[15px] text-gray-600 mt-1 font-medium">{pendingBooking.booking_date}</p>
                  </div>
                  <div className="text-right">
                    <p className="font-display-sm text-[28px] font-black text-gray-900">{pendingBooking.start_time.slice(0,5)}</p>
                    <p className="font-body-md text-[15px] text-gray-500 font-bold">to {pendingBooking.end_time.slice(0,5)}</p>
                  </div>
                </div>
                ...
                <div className="flex gap-4 mt-2">
                  ...
                  <button 
                    className="flex-1 bg-white border-2 border-red-500 text-red-500 hover:bg-red-50 h-14 rounded-xl font-button text-[16px] font-bold transition-colors active:scale-95"
        ...
        <section className="grid grid-cols-2 gap-5">
          ...
          <button 
            className="h-32 flex flex-col items-center justify-center gap-3 rounded-3xl transition-all active:scale-95 bg-white/70 backdrop-blur-xl border-2 border-white text-gray-800 shadow-[0_8px_24px_rgba(0,0,0,0.05)] hover:shadow-[0_12px_32px_rgba(0,0,0,0.08)] hover:-translate-y-1 hover:border-primary/30"
            onClick={() => router.push('/scan')}
          >
        ...
        <section>
          <div className="flex justify-between items-center mb-5">
            <h2 className="font-headline-md text-[22px] font-extrabold text-gray-900">Recent Bookings</h2>
          </div>
          <div className="space-y-4">
            {bookings.length === 0 && (
              <div className="bg-white/60 backdrop-blur-xl border border-white shadow-sm rounded-3xl p-8 text-center">
                <p className="font-body-md text-[16px] text-gray-500 font-medium">No bookings yet.</p>
                <button 
