...
export class BookingsService {
  ...
  async createBooking(stu_id: string, courtId: number, date: string, startTime: string) {
    ...
    // Validate the start time so the 1-hour slot can never overflow past 23:00
    // (BUG #06: e.g. a 23:00 start previously produced an invalid 24:00:00 end time).
    if (
      Number.isNaN(hours) || Number.isNaN(minutes) ||
      hours < 0 || minutes < 0 || minutes > 59
    ) {
      throw new BadRequestException('Invalid start time. Expected HH:MM.');
    }
    if (hours >= 23) {
      throw new BadRequestException('Bookings must start no later than 22:00 (each slot is one hour).');
    }
